...
export default function ContactPage() {
  ...
  // Reset submitted when the page loads (or user navigates back)
  useEffect(() => {
    setSubmitted(false);
  }, []);
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center p-4 pb-[120px]">
      <div className="border border-zinc-300 bg-[radial-gradient(circle_at_top_left,_rgba(26,75,207,1),_transparent_60%)] rounded-sm cursor-default">
        <div className="grid md:grid-cols-[1fr_220px] min-h-[600px] md:min-h-0">
              ...
              <div className="mt-12 flex flex-col gap-2 max-w-md">
                <div className="flex gap-3">
                  <div className="inline-block border border-zinc-300/60 bg-black/25 p-5">
                    {submitted ? (
                      <div className="confirmation-message p-6 bg-[#08131f] border border-[#39ff63] rounded-md text-center">
                        <h3 className="text-[#39ff63] text-lg font-bold mb-2">Thank You!</h3>
                        <p className="text-zinc-100 text-sm">
                          Your message has been sent successfully. I’ll get back to you as soon as possible.
                        </p>
                        <button
                          className="mt-4 border border-[#39ff63] text-[#39ff63] hover:bg-[#39ff63] hover:text-black transition-all duration-150 uppercase tracking-[0.25em] text-xs py-2 px-4 font-bold"
                      ...
                      <form onSubmit={onSubmit} className="space-y-5">
                        <div className="grid sm:grid-cols-2 gap-5">
                          <div className="flex flex-col gap-2 w-[400px]">
                            ...
                            <input
                              ...
                              className="w-full bg-[#08131f] border border-zinc-500/40 text-zinc-100 px-4 py-3 text-xs tracking-[0.12em] outline-none focus:border-[#39ff63] focus:shadow-[0_0_12px_rgba(57,255,99,0.25)] transition-all"
                              required
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-5">
                          <div className="flex flex-col gap-2 w-[400px]">
                            ...
                            <textarea
                              ...
                              className="w-full bg-[#08131f] border border-zinc-500/40 text-zinc-100 px-4 py-3 text-xs tracking-[0.12em] outline-none focus:border-[#39ff63] focus:shadow-[0_0_12px_rgba(57,255,99,0.25)] transition-all resize-none"
                              rows={5}
                              required
                            />
                          </div>
                        ...
                        <button
                          type="submit"
                          className="w-full sm:w-auto border border-[#39ff63] text-[#39ff63] hover:bg-[#39ff63] hover:text-black active:bg-[#2ecc58] active:text-black active:translate-y-[1px] transition-all duration-150 uppercase tracking-[0.25em] text-xs py-3 px-8 font-bold inline-flex items-center justify-center select-none cursor-pointer"
                        >
                          Send Message
                        </button>

                        {result && result !== "Sending...." && (
                          <p className="text-xs mt-2 text-[#39ff63]">{result}</p>
                        )}
                        {result === "Sending...." && (
                          <p className="text-xs mt-2 text-zinc-300">{result}</p>
                        )}
                      </form>
                    )}
                  </div>
          ...
          <aside className="border border-zinc-300/60 bg-gradient-to-b from-[#08131f] via-[#1a4bcf] to-[#06111b] p-6 flex flex-col justify-between text-zinc-100 min-h-full">
            ...
            <button className="mt-12 border border-[#39ff63] text-[#39ff63] hover:bg-[#39ff63] hover:text-black transition-all duration-200 uppercase tracking-[0.25em] text-xs py-3 font-bold cursor-pointer">
              My Apps
            </button>
          </aside>
        </div>
